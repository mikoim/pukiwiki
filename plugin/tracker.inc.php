...
function plugin_tracker_getlist($page,$refer,$config_name,$list,$order='',$limit=NULL)
{
	...
	//pkwk_log('AAAA');
	//pkwk_log(print_r(['page:' => $page, 'refer' => $refer, 'config_name' => $config_name, 'list' => $list, 'order' => $order], true));
	...
	if ($cache_enabled && file_exists($cache_filepath)) {
		...
		if ($json_cached) {
			...
			if (is_array($wrapdata)) {
				if (isset($wrapdata['version'], $wrapdata['html'], $wrapdata['refreshed_at'])) {
...
class Tracker_list
{
	...
	function get_all_links() {
		...
		pkwk_log('LINK');
		pkwk_log(print_r(array_keys($ar), true));
